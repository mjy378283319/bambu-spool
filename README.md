...
**文字不重复**：副行（品牌 · 材质 · 外观）与页脚的颜色名只补名字里没写的信息——
名字叫「魔创 PLA 天蓝色」时就不再印「魔创 · PLA」和「天蓝色」；全写全了副行整行省略，
余量主行上移补位。0.3.5 起也去掉了右侧的余量百分比和「偏低」标记（打印出来不直观）。
...
# 1 位光栅打包、ESC/POS 报文、对话框装配与标签实际渲染尺寸（82 项断言，node 直跑，不需要浏览器）
