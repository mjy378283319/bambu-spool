...
录料盘时点色块即填色名与色值，内置 **13 个品牌 · 173 个系列 · 1960 个色号**：

| 品牌 | 系列 | 色号 | 色值来源 |
|---|---|---|---|
| Polymaker | 3 | 104 | 官方公布 |
| Kexcelled | 5 | 200 | 商品图提色（近似） |
| 拓竹 | 38 | 240 | 官方 Hex Code Table（24 系列 PDF 原文 / 其余色片提色） |
| 大简 | 1 | 40 | 官方店 SKU |
| 兰博 | 30 | 213 | 商品图提色（近似） |
| 魔创 | 8 | 152 | 淘宝官方店 SKU |
| 锐造 | 12 | 122 | 商品图提色（近似） |
...
- **外观（表面工艺）**：与材料、颜色并列，15 种预设（普通 / 亮面 / 哑光 / 磨砂 / 丝绸 / 珠光 /
  ...
  **两种预填都只在你没选过时动手**；列表里按色调显示（普通灰 / 丝绸紫 / 哑光深色 / 荧光绿），类型与外观列都能点表头排序。
...
每盘料有二维码标签，扫一下直接跳到那盘料详情。入口：库存页「扫码」、槽位绑定弹窗「相机扫码」、或用手机自带相机扫贴纸上的网址。
...
改动即时存本地；另有「批量 A4 拼版」。版式是「左边文字 + 右边二维码」，**没有色块**（热敏纸只有黑白），颜色信息以 `#147DB5` 留在页脚。
...
| 风扇通道名 | 部件 / 右(辅助) / 左(辅助) / 外排，与拓竹官方 App「空调系统」页四行逐字一致（X1/P1/A1 等无自适应风道组件的机型仍是部件冷却 / 辅助部件冷却 / 腔体 / 热端）。左(辅助) 与外排是选配件：**装着但读不到转速写「未转」，真没装才写「未安装」**——判定要算上 `modeList` 里列过的部件 id，因为没转时固件可能压根不报这一路 |
| P2S 辅助风扇报在哪 | 不在 `big_fan1`（真机报文恒 0），而在 `device.airduct.parts` 的 `func==0` 部件，其 `state` **本身即百分比**，不能再除 15；右(辅助) 按 `id==16`、左(辅助) 按 `id==160` 认 |
| 仓温 | P2S 在 `device.ctc.info.temp`（32 位打包：低 16 位当前、高 16 位目标），X1 在 `print.chamber_temper`；后者为 0 / 缺失时让位给前者（有的固件会带上这个键却常年报 0） |
| 增量报文 | 打印机常只推变化的那几个键，而 `print.device` 是多层嵌套：**必须递归合并**——浅合并一旦碰到带 `device` 的增量就把 `device.ctc` 一起换掉，仓温偶发「—」就是这个原因 |
...
并给容器加 `REQUIRE_HTTPS=true`。登录接口有失败锁定（默认 5 次 / 15 分钟），密码走 PBKDF2 存储，会话 Cookie 为 HttpOnly。
...
python tests/test_fans_temps.py        # 风扇 0-15 档位换算、仓温两个来源、左(辅助) 装没装
python tests/test_hub_merge.py         # 增量报文递归合并、左(辅助) 记忆（仓温丢失的根因）
...
打印成果图、槽位绑定下拉自愈、刷新后停留在当前视图、全站字号与间距放大、增量报文递归合并
（修 P2S 仓温偶发「—」）、选配风扇「未安装 / 未转」区分、概览环形图可点、类型与外观排序与外观色调芯片。
