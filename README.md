...
- 点品牌色卡里的颜色时，**系列名写着外观就一起带过来**：点「PLA 哑光 色卡」里的色块，
  外观自动填成哑光。这两种预填都**只在当前是空或「普通」时**才动手，你自己选过的丝绸 /
  磨砂绝不会被改掉（被改掉过一次，见 0.3.4 的修复），有变动时会在色卡下方说明一句；
- 英文写法也认：matte / silk / translucent / glow / carbon / rainbow …
- 库存页的「外观」筛选跟着实际数据走，老数据里的自定义写法也能筛出来。
- 改外观时，料盘名若还是**自动生成**的那个（「品牌 材料 [外观] 颜色」）会跟着一起更新——
  否则会出现「外观列写哑光、名字里没有哑光」；自己起过名的料盘一个字都不动。
...
**左侧文字固定四行（0.3.6 按反馈定稿）**：① 品牌 → ② 类型 · 外观（普通不写）→
③ 名字（从料盘名里剥掉品牌/材料，剩下的通常是颜色名；手动起的名原样保留）→
④ 余量 `余 x g / 总量 g`。有「位置」就再补一行；页脚（编号 · 色值）固定在最下。
各行按实际行数**纵向均分铺满**，行数少间距自动变大，不会留出一大块空白；
0.3.5 起还去掉了右侧的余量百分比和「偏低」标记（打印出来不直观），页脚也不重复
名字里已写过的颜色名。
...
# 外观字段归一 / 迁移回填、自定义品牌、区域显示、耗材汇总与时区切日（103 项断言）
...
# 以及料盘状态口径 / 表头排序 / 价格分档 / 汇总页环形图 / 行操作分两组
...
> （图片 404 了布局还在、温度浮标飘出卡片、hash 深链没跳转、环形图被全局 `svg{width:16px}`
> 压成小点、表格比卡片宽 72px 把按钮顶到卡片外面）。两者互补。
> 它还会扫 1440 / 1280 桌面宽度和 320 / 360 / 390 / 430 手机宽度，断言**任何视图都不横向溢出**
> —— 这类问题截图里往往只是「右边少了个按钮」，量一下才知道差多少。
...
- [x] 打印机状态卡两列都撑满（风扇状态挪到右栏底部，左栏照片卡吃掉多余高度，整块无空白）
- [x] 槽位绑定下拉在「没进过料盘库存」时也能选到料盘（启动即拉列表 + 弹窗自愈）
- [x] 外观保存链路修通（表单里的外观以前根本没进 payload，选了哑光存下来还是普通）
      + 点色卡带外观，且两种预填都不覆盖用户已选的值
