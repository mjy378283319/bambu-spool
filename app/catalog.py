...
#
# 数据来源分两类，**列表第一个是首选档**（新建料盘时自动带出来的那个）：
#   · 用户实测称重（2026-09-18 更新）：大简 239 / Polymaker 150 / 拓竹 239 /
#     魔创 220 / 兰博 160 / Kexcelled 239 / 爱酷乐 239。这些是真实上秤的数字，
#     优先于厂商规格 —— 同品牌不同批次/不同产线会换盘型，称重比标称可信。
#   · 官方规格或估算（未实测的品牌），注释里标明「估算」。
#
...
BRAND_SPOOL_WEIGHTS: dict[str, list[float]] = {
    "拓竹": [239.0, 190.0],            # 实测 239g；另一档 190g 可重复使用盘
    "Polymaker": [150.0, 220.0],      # 实测 150g（纸盘）；旧塑料盘 220g
    "大简": [239.0, 150.0],            # 实测 239g；纸盘约 150g
    "魔创": [220.0, 200.0],            # 实测 220g；另一档 200g 估算
    "兰博": [160.0, 200.0],            # 实测 160g；另一档 200g 估算
    "Kexcelled": [239.0, 240.0],       # 实测 239g（与 240g 规格同档）
    "爱酷乐": [239.0],                 # 实测 239g
    "爱丽兹 Allizz": [200.0],
    "锐造": [200.0],                   # 官网未公布空盘重量，估算值，建议称重校准
    "JAYO": [140.0, 200.0],            # 纸盘约 140g / 塑料盘约 200g（估算，建议称重校准）
